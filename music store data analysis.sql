...
select customer.customer_id , customer.last_name ,customer.first_name , sum(invoice.total) as total_spe 
from customer 
join invoice on customer.customer_id = invoice.customer_id  
GROUP BY customer.customer_id , customer.customer_id , customer.last_name ,customer.first_name
order by total_spe desc limit 1;
-- Write query to return the email, first name, last name, & Genre of all Rock Music listeners. 
-- Return your list ordered alphabetically by email starting with A

select distinct email,last_name ,first_name  from customer 
join invoice on customer.customer_id = invoice.customer_id
...
-- who is the most senior employee on job title 
